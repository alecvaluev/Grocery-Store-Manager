...
The application is a simulation of the grocery store manager.
It loads data from the file or manual entering and checks if the data are valid and in the range of valid numbers, if they are not manual entering is required. The program reads the product id and the quantity to add them into the current purchase cart. After items are selected, the program prints the summary of the current purchase with tax and total information. When the program is shut down, it generates the "End of day summary", where it indicates the total cost of sold items, number of sales, and average items per sale as well as a summary of stock left and the ranks of top sellers in each category.

## Sample Output
```
Sample output from the program is shown below.
Note that the input values are separated by commas and that you should do the same in your program.
